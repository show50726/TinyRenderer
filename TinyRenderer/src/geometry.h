...
template <size_t Dim, typename T> struct vec {
	...
	T& operator[] (const size_t i) { return const_cast<T&>(static_cast<const vec<Dim, T>&>(*this)[i]); }
...
template <typename T> struct vec<2, T> {
	...
	T& operator[] (const size_t i) { return const_cast<T&>(static_cast<const vec<2, T>&>(*this)[i]); }
...
template <typename T> struct vec<3, T> {
	...
	T& operator[] (const size_t i) { return const_cast<T&>(static_cast<const vec<3, T>&>(*this)[i]); }
...
template<size_t Dim, typename T> const vec<Dim, T> operator*(const float f, const vec<Dim, T>& v) {
	vec<Dim, T> res;
	for (int i = 0; i < Dim; i++) res[i] = f * v[i];
	return res;
}
